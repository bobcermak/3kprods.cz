...
const Navbar: FC<NavbarProps> = ({ logo, button, className, href, children }) => {
    ...
    return (
        <nav className={classes}>
            ...
            <Ul type="menu" className={`flex laptop:items-center absolute laptop:static top-full laptop:top-none left-0 laptop:left-none right-0 laptop:right-none mx-5 mt-6 laptop:m-0 rounded-[20px] laptop:rounded-b-none shadow-secondary laptop:shadow-none inner-shadow-primary inner-shadow-none laptop:justify-center bg-[linear-gradient(77deg,#1C2E55_0%,#243c91_25%,#4461ef_49%,#98a8f7_100%)] laptop:bg-none flex-col laptop:flex-row gap-6 laptop:gap-20 px-6 py-6 laptop:p-0 transition-all duration-550 ease-in-out laptop:opacity-100 ${isMenuOpen ? "opacity-100 translate-x-0" : "opacity-0 -translate-x-10 pointer-events-none"}`}>
                {children}
            </Ul>
