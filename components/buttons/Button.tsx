...
const Button: FC<ButtonProps> = ({ isLink = true, link = "#", variant = "primary", isArrow = true, onClick, children }) => {
  const baseStyles: string = `inline-block px-6 py-3 no-underline rounded-[2.5rem] text-body font-bold text-center uppercase cursor-pointer w-fit backdrop-blur-xl backdrop-saturate-150 border border-white/20 transition-colors duration-250 ease-in-out ${isArrow ? "group" : ""}`;
  ...
  const arrow = isArrow ? (
    <i className={`fa-solid fa-arrow-right ml-[.625rem] rounded-full transition-transform duration-250 ease-in-out group-hover:translate-x-[3px] ${variant === "primary" ? "text-white bg-light-blue" : "text-blue bg-green"} py-[0.375rem] px-[0.4375rem]`}></i>
  ) : null;
